...
|---|---|---|---|---|---|---|---|---|
| ultikits.kits.claim | Claim a named kit for the sender, subject to permission/level/economy/cooldown/one-time checks and inventory space — worked out the way the inventory fills (matching partial stacks topped up first, then empty slots, each holding at most the stack's own maximum stack size, so a larger stack takes several), and refused with `Inventory full` before any charge when they are not free; anything the inventory still cannot take is dropped at the player's feet with `Part of the kit did not fit in your inventory and was dropped at your feet.`, never discarded; on success delivers the kit's items and runs its configured player/console commands. For a PAID kit the price is withdrawn first and the withdrawal's own result is checked before anything is handed over, so a withdrawal that does not go through (the balance moved after the earlier affordability check, or the economy rejected the transaction) delivers no items, runs no commands, writes no claim row and replies `Payment failed - the kit was not claimed` — a distinct reply from the affordability check's own `Insufficient balance, requires N`. The server console gets a `WARNING` naming the kit, the player and the amount, throttled to the FIRST refusal for each kit in each server session (further refusals for that kit are silent, and the line says so; `/kits reload` re-arms it). On success the price is taken first, then the claim row is written, then the items are handed over, then the kit's player and console commands run. If the claim row cannot be written (a storage error), the claim is refused: a paid kit's price is refunded, nothing is given, no command runs, and the reply is `Your claim could not be recorded, so nothing was charged or given. Please try again.`; if the refund fails as well, the reply is `Your claim could not be recorded, so nothing was given, but returning your payment failed. An administrator can see the details in the server console.` and the console logs an ERROR naming the player, the kit and the amount. On the JSON storage backend a write reaches only an in-memory cache flushed to disk on a timer, so a disk failure there is not seen at claim time | command | `/kits claim <name>` | ultikits.kits.use | player | player | brief | KitCommands#onClaim, KitService#claimKit, KitServiceImpl#deliverKit |
| ultikits.kits.create | Create a new kit definition from the sender's own current inventory contents (air slots filtered out), using the first item's material as the icon and a plain `&f<name>` display name; the new kit file is created only after its content is in the write journal (see `ultikits.gui.kit-editor`): a failed write removes the part-written file, so it leaves no kit file and no kit, and the sender is told `Error creating kit` (red); after a crash during the create, the next start creates the whole file from the journal (a file placed at that name by hand in the meantime is not overwritten: the journal is kept and the console names it); a name that is not a plain file name (it contains `/`, `\`, `:` or starts with `.`) is refused as `Invalid kit name`; a name that a kit file on disk already loads as but that is not in the catalogue (its write is still pending, or it was placed by hand without a reload) is refused with `Kit name already exists: <name>` (red) and the file is not touched; a name that more than one file in the kits folder already loads as (even when none of them parsed) is refused without writing, with the `Kit <name> was not changed: …` line naming the files; additionally requires the sender hold `ultikits.kits.admin` (a hand-coded body check, not a declared `@CmdMapping` permission) | command | `/kits create <name>` | ultikits.kits.use | player | admin | brief | KitCommands#onCreate, KitService#createKit |
| ultikits.kits.delete | Delete a kit definition's on-disk YAML file and remove it from the in-memory catalogue — the catalogue entry only once the file is gone: when the file cannot be deleted (or the kits folder cannot be read), the kit stays loaded, the sender is told `Kit <name> was not deleted: its file could not be removed. The console names the file.` (red) and a console warning names the file's path; a write of the kit's file still pending in the write journal is completed first (or withdrawn, when the file is already gone), so the kit cannot come back at the next start and a file that then cannot be deleted is whole (when the journal can be neither completed nor withdrawn the kit is not deleted); when more than one file in the kits folder loads as the kit (for example `VIP.yml` and `vip.yml` on a case-sensitive file system), no file is deleted, the kit stays loaded and the sender is told `Kit <name> was not changed: more than one file in the kits folder defines it (<files>). Keep one of these files, rename or remove the others, then run /kits reload.` (red), `<files>` the file names sorted and comma-separated; additionally requires the sender hold `ultikits.kits.admin` (hand-coded body check); reachable from console (no method-level `@CmdTarget`, stays at the class's `BOTH` default) | command | `/kits delete <name>` | ultikits.kits.use | both | admin | brief | KitCommands#onDelete, KitService#deleteKit |
| ultikits.kits.edit | Open the kit editor GUI (`ultikits.gui.kit-editor`) for a named kit, pre-filled with its current items; a kit that more than one file defines is not opened, and the sender gets the same `Kit <name> was not changed: …` line as `ultikits.kits.delete`; additionally requires the sender hold `ultikits.kits.admin` (hand-coded body check) | command | `/kits edit <name>` | ultikits.kits.use | player | admin | brief | KitCommands#onEdit |
...
| ultikits.kits.reload | Reload every kit definition from disk (`kits/*.yml`), replacing the in-memory catalogue entirely; when more than one file loads as one kit, the last one listed that parses is loaded (as before, so it can still be claimed) and the console warns once, naming the kits folder, every such file and the one loaded, and that the kit cannot be edited or deleted until only one remains; additionally requires the sender hold `ultikits.kits.admin` (hand-coded body check); reports the reloaded kit count; reachable from console (no method-level `@CmdTarget`, stays at the class's `BOTH` default) | command | `/kits reload` | ultikits.kits.use | both | admin | brief | KitCommands#onReload, KitService#reload |
...
| ultikits.gui.kit-editor | Chest GUI for editing a kit's item contents directly by moving real items into/out of a 45-slot grid (slots 0-44), pre-filled with the kit's existing items on open. **The grid half is not delivered today:** `KitEditorGui` overrides neither `onClick` nor `onDrag`, so the GUI library cancels every click on slots 0-44 and no item can be moved into, out of or within the grid (`UltiKits/UltiKits#16`, open), which leaves Save re-saving the items the editor opened with; a Save button (slot 45) collects whatever is currently in those 45 slots and overwrites the kit's stored item data, rewriting the kit file in place after first writing the new content to a write journal in the module's `kit-journal` folder (beside the `kits` folder); the kit file is never replaced or moved, so its permissions, owner, access-control list, links and other attributes stay as they were, and a kit file the server may not write, or a symbolic link to a missing file, fails the save before anything is written. A save that fails part-way puts the old content back, so the file is as it was and the loaded kit keeps its items (the button answers `Error saving kit`); if the old content cannot be put back either, the journal is kept and the console says the new content is written at the next start or `/kits reload`, and a later save of that kit first completes it (or is refused while it cannot). After a crash during a save, the next start completes the write from the journal before the kits load (console: `Completed an interrupted write of kit file <path>.`); a journal that is damaged, or whose kit file no longer exists, is logged and discarded without changing any kit file, and one that cannot be read, or whose kit file cannot be checked, is kept. The write is completed only while the kit file holds its old content or part of the old or new content; a kit file changed by hand after the crash is not overwritten, and the journal is kept until it is deleted. A kit file whose write is still pending after the start is not loaded until the write is resolved (console: `Kit file <path> was not loaded: a write of it is still pending in <journal> …`). A save also fails when its journal cannot be written (the kit file is then untouched). Replay writes only a plain kit file and never follows a symbolic link: when the kit file is a link at that point (even the one the save went through), nothing is written, the journal is kept and the console says how to finish the write (replace the link with the file it points to) or drop it (delete the journal); saves and deletes of that kit wait until then. The `kit-journal` folder is part of the same trust boundary as the kits folder: whoever can write it can write kit files; a `kit-journal` that is itself a symbolic link is never used: saves and deletes fail, and nothing in it is replayed, until it is a plain folder. A save reported as failed whose old content was put back never takes effect later (unless the console says its journal could not be removed or emptied); one whose old content could not be put back is completed at the next start, `/kits reload` or the next save of that kit, as the console says (until then claims hand out the items loaded before). On POSIX systems the journal folder and its journals are created readable only by the server's own account (when the folder's permissions cannot be changed, the console warns and saving goes on). All of this holds unless more than one file in the kits folder defines the kit, in which case nothing is written and the button answers the `Kit <name> was not changed: …` line naming the files, or unless `config/config.yml: enabled` is `false`, in which case the write is refused at the save gateway (`KitServiceImpl#saveKitItems`) and the button answers `The kit system is currently disabled on this server`, which matters because this editor outlives the `/kits edit` command that opened it; a Cancel button (slot 53, hardcoded English lore text regardless of `language`) discards changes by simply closing the inventory — items placed into the grid before cancelling are NOT returned to the editor's own inventory automatically, they remain wherever Bukkit's own inventory-close handling puts them | gui | `ultikits.kits.edit` | n/a | n/a | admin | detailed | KitEditorGui#onOpen, KitEditorGui#handleSave |
