...
| ultikits.kits.create | Create a new kit definition from the sender's own current inventory contents (air slots filtered out), using the first item's material as the icon and a plain `&f<name>` display name; a name holding `/`, `\` or the step `..` is refused with `Invalid kit name <name>: a kit name cannot contain /, \ or ..` (red) before anything is written, and the file a new kit is written to must lie directly in the kits folder; a name that a file in the kits folder already loads as but that is not loaded (placed by hand without a reload, or unreadable, including a case variant such as `VIP.yml` for `vip`) is refused with `Kit <name> was not created: a file for it already exists in the kits folder (<file>). Load it with /kits reload, or remove it first.` (red) and the file is not touched, also when the file appears while the command runs; a name that more than one file in the kits folder already loads as (even when none of them parsed) is refused without writing, with the `Kit <name> was not changed: …` line naming the files; additionally requires the sender hold `ultikits.kits.admin` (a hand-coded body check, not a declared `@CmdMapping` permission) | command | `/kits create <name>` | ultikits.kits.use | player | admin | brief | KitCommands#onCreate, KitService#createKit |
