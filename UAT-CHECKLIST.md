...
| ultikits.gui.kit-browser.neg-disabled-mid-session | `language: en`; `config/config.yml: enabled` at its shipped default `true`; a FREE kit `starter` exists with no permission or level requirement, `reBuyable: true`, non-empty items, and `LaoJunClient` (the pixel-layer test player) has enough empty inventory slots for it; the browser opened via `ultikits.kits.open` WHILE the switch was still `true`, and left open | Without closing the browser, set `enabled: false` in the file and run `/ul reload UltiTools-Kits` from the console (NOT a server restart, which would close the inventory on its own and prove nothing); then click the `starter` icon in the still-open browser; then close the browser and run `/kits` | The click produces `The kit system is currently disabled on this server` (red) and nothing else: no `Successfully claimed kit: starter`, no items added to `LaoJunClient`'s inventory, and no new or changed `kits_claims` row for that player and `starter` — read the table before and after the click and compare, since `reBuyable: true` means a claim would show as a `claim_count` increment rather than as a new row. The inventory stays OPEN: enforcing the switch does not close a window the player already had on screen. The `/kits` afterwards is refused with the same message, confirming the switch was really off for the whole step rather than the click having failed for some other reason | pixel | KitBrowserGui |
| ultikits.gui.kit-editor | `language: en`; `LaoJunClient` (the pixel-layer test player) holds `ultikits.kits.admin`; a kit `test1` exists with at least one item already stored; the editor open via `ultikits.kits.edit` | Move the pre-filled item to a different slot within the 45-item grid, add one new item from the player's own inventory into an empty grid slot, then click the Save button (slot 45); separately (re-open, fresh attempt), instead click the Cancel button (slot 53, hardcoded English "Click to save kit contents" lore regardless of `language` — this string appears on the SAVE button's lore, not Cancel's own, a naming mismatch worth noting verbatim as read, not corrected) without touching Save | Save: chat shows `Saved kit: test1` (green); `test1.yml`'s `items` field now CONTAINS both the moved original item and the newly added one — `KitEditorGui#handleSave` walks slots 0-44 in order and collects only non-air items into a dense, gap-free list (`saveKitItems` serializes that compacted array), so the saved data reflects PRESENCE and RELATIVE ORDER, not the items' literal absolute grid slot numbers; claiming this kit afterward delivers both items via `Inventory#addItem` (which itself ignores original slot position). Cancel: the inventory simply closes with no chat message and no file write — `test1.yml` is UNCHANGED from before this row, even though items were moved around in the grid before cancelling | pixel | KitEditorGui |
